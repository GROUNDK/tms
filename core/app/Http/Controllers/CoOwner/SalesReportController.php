...
class SalesReportController extends Controller
{
    ...
    public function sale()
    {
        ...
        if(request()->has('search') && request()->search != ''){
            ...
            $data['sales']      = $this->co_owner->owner->bookedTickets()
                                ->with('trip', 'trip.route', 'counterManager')
                                ->where('id', "$keyword")
                                ->whereStatus(1)->paginate(getPaginate());
        }else{
            $data['sales']          = $this->co_owner->owner->bookedTickets()->with('trip', 'trip.route', 'counterManager')->whereStatus(1)->paginate(getPaginate());
        }
        $data['empty_message']  = "No data found";
        $data['owner']          = $this->co_owner->owner;
        $data['co_owner']       = $this->co_owner;

        $data['routes']         = $this->co_owner->owner->routes()->whereStatus(1)->get();
        $data['trips']          = $this->co_owner->owner->trips()->get();
        $data['counterManagers']= $this->co_owner->owner->counterManagers()->with('counter')->where('status', 1)->orderBy('name')->get();


        return view('co-owner.report.sale', $data);
    }

    public function saleDetail($id)
    {
        $data['page_title']     = "Booking Details";
        $data['sale']           = $this->co_owner->owner->bookedTickets()->with('trip', 'trip.route', 'counterManager')->whereId($id)->whereStatus(1)->first();
        $data['empty_message']  = "No data found";
        $data['owner']          = $this->co_owner->owner;
        $data['co_owner']       = $this->co_owner;
        return view('co-owner.report.sale_detail', $data);
    }
    ...
    public function filterSales(Request $request)
    {
        ...
        $query = $this->co_owner->owner->bookedTickets();
        ...
        return redirect()->route('co-owner.report.sale.filtered')->with( ['filteredData' => $data] );
    ...
    public function filteredData(Request $request)
    {
        ...
        if($sales == null){
            return redirect()->route('co-owner.report.sale');
        }
