...
class ResetPasswordController extends Controller
{
     ...
     * @var string
     */
    public $redirectTo = '/co-owner/dashboard';
    ...
    {
        $this->middleware('co-owner.guest');
    }
    ...
    public function showResetForm(Request $request, $token)
    {
        ...
        if (empty($tk)) {
            ...
            return redirect()->route('co-owner.password.reset')->withNotify($notify);
        }

        $email = $tk->email;
        return view('co-owner.auth.passwords.reset', compact('page_title', 'email', 'token'));
    }
    ...
    public function reset(Request $request)
    {
        ...
        $user = CoOwner::where('email', $reset->email)->first();
        if ($reset->status == 1) {
            $notify[] = ['error', 'Invalid code'];
            return redirect()->route('co-owner.login')->withNotify($notify);
        }
