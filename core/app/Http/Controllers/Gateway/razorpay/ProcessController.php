...
class ProcessController extends Controller
{
    ...
    public static function process($deposit)
    {
        ...
        $val['name']            = Auth::guard('owner')->user()->username;
        $val['description']     = "Payment By Razorpay";
        $val['image']           = asset( 'assets/images/logoIcon/logo.png');
        $val['prefill.name']    = Auth::guard('owner')->user()->owner_name;
        $val['prefill.email']   = Auth::guard('owner')->user()->email;
        $val['prefill.contact'] = Auth::guard('owner')->user()->mobile;
