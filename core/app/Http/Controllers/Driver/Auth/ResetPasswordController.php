...
class ResetPasswordController extends Controller
{
    ...
    public function showResetForm(Request $request, $token)
    {
        ...
        if (empty($tk)) {
            ...
            return redirect()->route('driver.password.reset')->withNotify($notify);
        }

        $email = $tk->email;
        return view('driver.auth.passwords.reset', compact('page_title', 'email', 'token'));
    }
    ...
    public function reset(Request $request)
    {
        ...
        $user = Driver::where('email', $reset->email)->first();
        if ($reset->status == 1) {
            $notify[] = ['error', 'Invalid code'];
            return redirect()->route('driver.login')->withNotify($notify);
        }
