                                ...
                                <div class="left">
                                    <h5>
                                        @if($ticket->owner_id != null)
                                            <p class="d-inline"><a href="{{route('admin.users.detail', $ticket->owner_id)}}" >&#64;{{ $ticket->name }}</a></p>
                                        @else
                                            <p>{{ $ticket->name }}</p>
                                        @endif
                                        <p class="d-inline"> - {{ diffForHumans($message->created_at) }}</p>
                                    </h5>
                                </div>
                                ...
                                @if($message->attachments()->count() > 0)
                                    <div class="my-3">
                                        @foreach($message->attachments as $image)
                                            <a href="{{route('admin.ticket.download',encrypt($image->id))}}" class="mr-3"><i class="fa fa-file"></i> @lang('Attachment') {{ $loop->iteration }} </a>
                                        @endforeach
                                    </div>
                                @endif
                            </div>
