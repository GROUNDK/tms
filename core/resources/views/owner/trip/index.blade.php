                        ...
                        <table class="table table--light style--two">
                            ...
                            <tr>
                                ...
                                <td data-label="@lang('Action')">
                                    <a href="@if($trip->trashed())javascript:void(0) @else {{route('owner.trip.edit', $trip->id)}}@endif"   data-trip="{{ $trip }}" class="icon-btn" data-toggle="tooltip" data-placement="top" title="@lang('Edit')">
...
<div class="row d-flex flex-row-reverse p-2">
            ...
            <div class="input-group has_append">
                ...
                <div class="input-group-append">
                    ...
                    @if(request()->has('search'))
                        <a href="{{ route('owner.trip.index') }}" class="btn btn--dark rounded-right ">
                            @lang('Clear')
                        </a>
                    @endif
                </div>
    ...
    <div class="px-1 col-xl-3 col-lg-12 mb-2 mb-xl-0">
        @if(request()->routeIs('owner.trip.index'))
        <a href="{{ route('owner.trip.create') }}" class="btn btn--success mr-1 mb-2 mb-xl-0 btn-block">
            <la class="la la-plus"></la>@lang('Add New')
        </a>
        @else
            @if(request()->routeIs('owner.trip.trashed.search'))
            <a href="{{route('owner.trip.trashed')}}" class="btn btn--dark"><i class="la la-reply"></i>@lang('Back')</a>
            @else
    ...
    <div class="px-1 col-xl-3 col-lg-12 mb-2 mb-xl-0">
        @if(request()->routeIs('owner.trip.index'))
        <a href="{{ route('owner.trip.trashed') }}" class="btn btn-danger btn-block"><i class="fas fa-trash-alt"></i>@lang('Trashed')</a>
        @endif
    </div>
</div>
@endpush
    ...
    <script>
        ...
        (function($){
            $(document).on('click', '.delete-btn', function () {
                ...
                var id  = $(this).data('id');
                var link  = `{{ route('owner.trip.remove', '') }}/${id}`;
