                    ...
                    <div class="table-responsive--lg  table-responsive">
                        <table class="table table--light style--two">
                            ...
                            <tr>
                                ...
                                <td data-label="@lang('Nick Name')"> {{ $a_bus->bus->nick_name }} </td>
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">@lang('Add Bus to Trip')
                    </h5>
                    <div class="text-right">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close" data-toggle="tooltip" title="@lang('Close')">
                            <span aria-hidden="true">&times;</span>
                        </button>
                        <a href="{{route('owner.trip.bus.create')}}" class="close" data-toggle="tooltip" title="@lang('Open in New Page')"><span aria-hidden="true">&#10064;</span></a>
                    </div>
                </div>
                <div class="modal-body">

                    <form action="{{ route('owner.trip.bus.store', 0) }}" method="POST">
                        @csrf
...
<div class="row d-flex flex-row-reverse p-2">
        ...
        <form action="" method="GET" class="bg-white">
            <div class="input-group has_append">
                ...
                <div class="input-group-append">
                    ...
                    <a href="{{ route('owner.trip.bus.index') }}" class="btn btn--dark rounded-right ">@lang('Clear')</a>
                    @endif
                </div>
            </div>
        </form>
    ...
    <div class="px-1 col-xl-3 col-lg-12 mb-2 mb-xl-0">
        @if(request()->routeIs('owner.trip.bus.index'))
        <button class="btn btn--success mr-1 mb-2 mb-xl-0 btn-block add-btn">
            <i class="las la-plus"></i>@lang('Add New')
        </button>
        @else
            @if(request()->routeIs('owner.trip.bus.trashed.search'))
            <a href="{{route('owner.trip.bus.trashed')}}" class="btn btn-dark"><i class="la la-reply"></i>@lang('Back')</a>
            @else
            <a href="{{route('owner.trip.bus.index')}}" class="btn btn-dark"><i class="la la-reply"></i>@lang('Back')</a>
            @endif
        @endif
    </div>
    <div class="px-1 col-xl-3 col-lg-12 mb-2 mb-xl-0">
        @if(request()->routeIs('owner.trip.bus.index'))
        <a href="{{ route('owner.trip.bus.trashed') }}" class="btn btn-danger d-block"><i class="fas fa-trash-alt"></i>@lang('Trashed')</a>
        @endif
    </div>
    ...
    <script>
        ...
        (function($){
            ...
            $(document).on('click', '.edit-btn', function () {
                ...
                var link  = `{{ route('owner.trip.bus.store', '') }}/${data.id}`;
                ...
                var elink = `{{route('owner.trip.bus.edit','')}}/${data.id}`
            ...
            $(document).on('click', '.delete-btn', function () {
                ...
                var id  = $(this).data('id');
                var link  = `{{ route('owner.trip.bus.remove', '') }}/${id}`;

                var action_type = $(this).data('action_type');
                if(action_type == 'delete'){
                    modal.find('.modal-body').text("{{ trans('Are you sure to delete this Assigned Bus')}}");
                }else{
                    modal.find('.modal-body').text("{{ trans('Are you sure to restore this Assigned Bus?')}}");
                }
