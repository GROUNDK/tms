        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form action="{{ route('owner.trip.schedule.store', 0) }}" method="POST">
                        @csrf

                        <div class="form-group">
                            <label>@lang('Starts From') <span class="text-danger">*</span></label>
                            <div class="input-group clockpicker">
                                <input type="text" class="form-control" placeholder="--:--" name="starts_from" autocomplete="off">
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group clockpicker">
                                <input type="text" class="form-control" placeholder="--:--" name="ends_at" autocomplete="off">
                            </div>
                        </div>
...
@push('breadcrumb-plugins')
    @if(request()->routeIs('owner.trip.schedule'))
    <button data-toggle="modal" data-target="#addModal" class="btn btn--success btn-sm box--shadow1 text-small">
        <i class="las la-plus"></i>@lang('Add New')
    </button>
    @else
        <a href="{{route('owner.trip.schedule.index')}}" class="btn btn-dark"><i class="la la-reply"></i>@lang('Back')</a>
    @endif
    @if(request()->routeIs('owner.trip.schedule.index'))
        <a href="{{ route('owner.trip.schedule.trashed') }}" class="btn btn-danger d-block"><i class="fas fa-trash-alt"></i>@lang('Trashed')</a>
    @endif
@endpush
    ...
    <script>
        ...
        (function($){
            $(document).on('click', '.edit-btn', function () {
                ...
                var link  = `{{ route('owner.trip.schedule.store', '') }}/${id}`;
            ...
            $(document).on('click', '.delete-btn', function () {
                ...
                var link  = `{{ route('owner.trip.schedule.remove', '') }}/${id}`;
                modal.find('form').attr('action', link);
                modal.modal('show');
            });
