            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <div class="text-right">
                        ...
                        <a href="{{route('co-owner.supervisor.create')}}" class="close" data-toggle="tooltip" title="@lang('Open in New Page')"><span aria-hidden="true">&#10064;</span></a>
                    </div>
                </div>
                <div class="modal-body">

                    <form action="{{ route('co-owner.supervisor.store', 0) }}" method="POST">
                        @csrf
...
@push('breadcrumb-plugins')
    ...
    @if(request()->routeIs('co-owner.supervisor.index'))
        <button data-toggle="modal" data-target="#addModal" class="btn btn-sm btn--success text--small">
            <i class="fas fa-plus"></i>@lang('Add New')
        </button>
    @else
        <a href="{{route('co-owner.supervisor.index')}}" class="btn btn-sm btn--dark text--small box--shadow1"><i class="fas fa-reply"></i>@lang('Back')</a>
    @endif

    @if(request()->routeIs('co-owner.supervisor.index'))
    <a href="{{ route('co-owner.supervisor.trashed') }}" class="btn btn-sm btn--danger text--small box--shadow1"><i class="fas fa-trash-alt"></i>@lang('Trashed')</a>
    @endif
    ...
    <script>
        ...
        (function($){
            ...
            $(document).on('click', '.edit-btn', function () {
                ...
                var link  = `{{ route('co-owner.supervisor.store', '') }}/${data.id}`;
                ...
                var elink = `{{route('co-owner.supervisor.edit','')}}/${data.id}`
            ...
            $(document).on('click', '.delete-btn', function () {
                ...
                var link  = `{{ route('co-owner.supervisor.remove', '') }}/${id}`;
                modal.find('form').attr('action', link);
                modal.modal('show');
            });
        })(jQuery)
