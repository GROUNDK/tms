                        ...
                        <table class="default-data-table table table-striped">
                            ...
                            <tr>
                                ...
                                <td data-label="@lang('Name')">
                                    <div class="user">
                                        <div class="thumb">
                                            <a href="{{ getImage('assets/owner/images/driver/'. $driver->image, true)}}" class="image-popup">
                                                <img src="{{ getImage('assets/owner/images/driver/'. $driver->image, true)}}" alt="image">
                                            </a>
                                            <span class="name">{{$driver->name}}</span>
                                        </div>
                                    </div>
                                </td>
                                <td data-label="@lang('Username')">{{ $driver->username }}</td>

                                <td data-label="@lang('Email')">{{ $driver->email }}</td>

                                <td data-label="@lang('Status')">
                                <span class="text--small badge font-weight-normal badge--{{$driver->status?'success':'danger'}}">
                                        {{ $driver->status? trans('Active'):trans('Inactive') }}
                                    </span>
                                </td>
                                <td data-label="@lang('Action')">
                                    <a href="javascript:void(0)" data-driver="{{ $driver }}" class="icon-btn {{ $driver->trashed()?'disabled':'edit-btn' }}" data-toggle="tooltip" data-placement="top" title="@lang('Edit')">
                                        <i class="la la-pencil"></i>
                                    </a>

                                    <a href="javascript:void(0)" data-id="{{ $driver->id }}" class="ml-1 icon-btn btn--{{$driver->trashed()?'success':'danger'}} delete-btn {{ $driver->status==0?'disabled':'' }}" data-toggle="tooltip" data-placement="top" data-action_type="{{$driver->trashed()?'restore':'delete'}}" title="{{$driver->trashed()? trans('Restore'): trans('Delete')}}">
                                        <i class="la la-trash{{$driver->trashed()?'-restore':''}}"></i>
                                    </a>
                                </td>
                            </tr>
                            @empty
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">@lang('Add Driver')
                    </h5>
                    <div class="text-right">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close" data-toggle="tooltip" title="@lang('Close')">
                            <span aria-hidden="true">&times;</span>
                        </button>
                        <a href="{{route('co-owner.driver.create')}}" class="close" data-toggle="tooltip" title="@lang('Open in New Page')"><span aria-hidden="true">&#10064;</span></a>
                    </div>
                </div>
                <div class="modal-body">

                    <form action="{{ route('co-owner.driver.store', 0) }}" method="POST">
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="username" id="username" class="form-control" placeholder="Type Here..."/ autocomplete="off"  required/>
                        ...
                        <div class="form-group">
                            <button type="submit" class="btn btn-block btn--primary">@lang('Add Driver')</button>
                        </div>
                    </form>
...
@push('breadcrumb-plugins')
        ...
        @if(request()->routeIs('co-owner.driver.index'))
        ...
        @endif
        @if(request()->routeIs('co-owner.driver.index'))
        <a href="{{ route('co-owner.driver.trashed') }}" class="btn btn-sm btn--danger text--small box--shadow1"><i class="fas fa-trash-alt"></i>@lang('Trashed')</a>
        @endif
    ...
    <script>
        ...
        (function($){
            ...
            $(document).on('click', '.edit-btn', function () {
                ...
                var link  = `{{ route('co-owner.driver.store', '') }}/${data.id}`;
                ...
                var elink = `{{route('co-owner.driver.edit','')}}/${data.id}`
            ...
            $(document).on('click', '.delete-btn', function () {
                ...
                if(action_type == 'delete'){
                    modal.find('.modal-body').text("{{ trans('Are you sure to delete this Driver?')}}");
                }else{
                    modal.find('.modal-body').text("{{ trans('Are you sure to restore this Driver?')}}");
                }
                var link  = `{{ route('co-owner.driver.remove', '') }}/${id}`;
                modal.find('form').attr('action', link);
                modal.modal('show');
            });
        })(jQuery)
